...
import { initializeR2Settings } from '../managers/r2-settings';
import { initializeGithubSettings } from '../managers/github-settings';
...
document.addEventListener('DOMContentLoaded', async () => {
	...
	const targetSection = (initialSection === 'general' || initialSection === 'interpreter' || initialSection === 'properties' || initialSection === 'highlighter' || initialSection === 'reader' || initialSection === 'r2' || initialSection === 'github') ? initialSection : 'general';
	...
	async function initializeSettings(): Promise<void> {
		try {
			...
			// Initialize R2 settings
			try {
				await initializeR2Settings();
			} catch (error) {
				console.error('Error initializing R2 settings, continuing with defaults:', error);
			}

			// Initialize GitHub settings
			try {
				await initializeGithubSettings();
			} catch (error) {
				console.error('Error initializing GitHub settings, continuing with defaults:', error);
			}
	...
	async function handleUrlParameters(): Promise<void> {
		...
		if (section === 'general' || section === 'interpreter' || section === 'properties' || section === 'highlighter' || section === 'reader' || section === 'r2' || section === 'github') {
