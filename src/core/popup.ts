...
import { saveToGithub, isGithubConfigured } from '../utils/github-uploader';
...
function setClipButtonProcessing(processing: boolean): void {
	const clipButton = document.getElementById('clip-btn');
	if (clipButton) clipButton.classList.toggle('processing', processing);
}
...
// Build the final markdown (frontmatter + body) from the popup DOM.
// Shared shape with handleSaveToDownloads so GitHub save and file save produce
// identical content.
async function buildFinalMarkdownFromPopupDOM(): Promise<string> {
	const properties = getPropertiesFromDOM();
	const noteContentField = document.getElementById('note-content-field') as HTMLTextAreaElement;
	const frontmatter = await generateFrontmatter(properties);
...
// Build a repository path "<dir>/<noteName>.md" from the popup DOM fields.
// - note name is sanitized via the same sanitizeFileName used for file save
// - avoids double ".md"
// - path prefix: trims surrounding "/", collapses "//", rejects ".." segments
function buildGithubPathFromPopupDOM(): string {
	const noteNameField = document.getElementById('note-name-field') as HTMLInputElement;
	const pathField = document.getElementById('path-name-field') as HTMLInputElement;

	let baseName = sanitizeFileName((noteNameField?.value || '').trim()) || 'untitled';
	if (!baseName.toLowerCase().endsWith('.md')) {
		baseName += '.md';
	}

	const dir = (pathField?.value || '')
		.split('/')
		.map(seg => seg.trim())
...
async function handleSaveToGithub() {
	if (!isGithubConfigured(generalSettings.githubConfig)) {
		showError('githubNotConfigured');
		return;
	}

	setClipButtonProcessing(true);
	try {
		const fileContent = await buildFinalMarkdownFromPopupDOM();
		const filePath = buildGithubPathFromPopupDOM();

		const result = await saveToGithub(fileContent, filePath, generalSettings.githubConfig);
		if (!result.success) {
			console.error('[GitHub Save] failed:', result.error);
			showError('failedToSaveGithub');
			return;
		}

		const tabInfo = await getCurrentTabInfo();
		await incrementStat('saveToGithub', '', filePath, tabInfo.url, tabInfo.title);
...
function determineMainAction() {
	...
	switch (loadedSettings.saveBehavior) {
		case 'copyToClipboard':
			...
			addSecondaryAction(secondaryActions, 'saveToGithub', handleSaveToGithub);
			break;
		case 'saveFile':
			...
			break;
		case 'saveToGithub':
			mainButton.textContent = getMessage('saveToGithub');
			mainButton.onclick = () => handleSaveToGithub();
			// Add direct actions to secondary
			addSecondaryAction(secondaryActions, 'addToObsidian', () => handleClipObsidian());
			addSecondaryAction(secondaryActions, 'saveFile', handleSaveToDownloads);
			break;
...
function getActionIcon(actionType: string): string {
	switch (actionType) {
		...
		case 'saveToGithub': return 'github';
		default: return 'plus';
	}
}
