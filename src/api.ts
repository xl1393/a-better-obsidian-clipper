...
export interface ClipOptions {
	...
	parsedDocument?: any;
	/**
	 * Optional hook to transform image URL (e.g. upload to remote storage).
	 * Return undefined to keep original URL.
	 * Hook should catch all errors internally and return undefined on failure.
	 */
	imageProcessor?: (imageUrl: string) => Promise<string | undefined>;
}
...
export async function clip(options: ClipOptions): Promise<ClipResult> {
	...
	// Process image through optional imageProcessor hook (e.g., R2 upload)
	if (options.imageProcessor && defuddleResult.image) {
		try {
			const newUrl = await options.imageProcessor(defuddleResult.image);
			if (newUrl) {
				defuddleResult.image = newUrl;
			}
		} catch {
			// Hook contract requires it not to throw, but defensive catch
		}
	}
