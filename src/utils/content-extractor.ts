...
import { createR2ImageProcessor, processContentImages } from './r2-image-processor';
import {
	getElementByXPath,
	wrapElementWithMark,
	wrapTextWithMark
...
export async function initializePageContent(
	...
	try {
		...
		// Optionally upload inline content images to R2 and rewrite their src to R2 URLs.
		// Runs before markdown conversion so the generated markdown references R2 URLs.
		// Never throws; images that fail to upload keep their original URL.
		content = await processContentImages(content, currentUrl);
		...
		// Optionally upload the cover image to R2 and swap in the public URL.
		// The processor never throws; on failure it returns undefined and we keep the original URL.
		const imageProcessor = createR2ImageProcessor(currentUrl);
		if (imageProcessor && image) {
			const uploadedUrl = await imageProcessor(image);
			if (uploadedUrl) {
				image = uploadedUrl;
			}
		}
